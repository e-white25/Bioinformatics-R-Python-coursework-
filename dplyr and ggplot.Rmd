...
subset <- data %>% ### Modify this code block by using the function select
  select(Samples, gene, geo_accession)
```
...
```{r}
gene <- data %>%
  filter(gene=='DPM1', tissue.ch1 != 'breast tumor')
```
...
```{r}
noncancer <- data %>%
  filter(FPKM > 200, tissue.ch1 != 'breast tumor') ### Modify this code block
...
data %>%
  filter(gene == 'BRCA1') %>%
  summarize(brca1_exp = mean(FPKM))### pipe your commands here
...
data %>%
  filter (gene == 'BRCA1' | gene == 'BRCA2') %>%
  group_by(tissue.ch1, metastasis.ch1, gene) %>%
  summarize(avg_exp_BRCA = mean(FPKM))
```
